...
public class Player extends Playable
{
	...
	public boolean useMagic(Skill skill, Item item, boolean forceUse, boolean dontMove)
	{
		...
		if (usedSkill.getAlternateEnemySkill() != null)
		{
			usedSkill = skill.getAlternateEnemySkill();
			
			final WorldObject target = getTarget();
			if ((target != null) && target.isPlayer())
			{
				final Player targetPlayer = target.getActingPlayer();
				if ((((getParty() != null) && (getParty() == targetPlayer.getParty())) || ((getClan() != null) && (getClan() == targetPlayer.getClan())) || ((getAllyId() != 0) && (getAllyId() == targetPlayer.getAllyId()))))
				{
					usedSkill = skill.getAlternateAllySkill();
				}
			}
		}
	...
	public void sendItemScore()
	{
		...
		{
			sendPacket(new ExItemScore(_itemScore.get()));
	...
	{
		if (item.getTemplate().getCrystalType() == CrystalType.NONE)
		{
			return;
		}
		
		_itemScore.addAndGet((int) Math.pow(1.2, item.getEnchantLevel()));
		sendItemScore();
	}
